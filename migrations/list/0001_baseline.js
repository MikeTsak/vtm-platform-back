// Baseline schema — this is the full table set as it existed before the
// migration runner was introduced. It only ever runs once per database
// (tracked in `schema_migrations`), so on an already-initialized database
// this executes exactly one time and is then skipped forever after.
//
// Left un-wrapped in a catch-all try/catch on purpose: if a CREATE TABLE
// fails partway through, the error must propagate so the runner does NOT
// record this migration as applied, and the next boot retries it.
module.exports = {
  name: '0001_baseline',
  async up(pool) {
    // Two narrow, self-contained column additions that predate the
    // migration runner. Kept as ALTER + catch (rather than a hard failure)
    // because they need to be harmless whether the column already exists
    // (live DB, added by hand previously) or not (fresh DB, added by the
    // CREATE TABLE below).
    try {
